...
function Card({
    ...
    const [commentsToShow, setCommentsToShow] = React.useState(comments)
    ...
    function submitComment() {
        httpCall('posts/comment', {postId: postId, text: commentText}, 'POST')
    ...
    return (
        <div className="card">
            ...
            <div className="bottom">
                <div className="actionBtns">
                    <div className="left">
                        <div>
                            <span className="like" onClick={() => {
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                     fill={hasLiked ? 'red' : '#000000'} xmlns="http://www.w3.org/2000/svg"
                        ...
                        <div>
                            ...
                            <span className="commentCount">{commentsToShow.length}</span>
                        </div>
                    </div>
                </div>
                ...
                <h4 onClick={() => setShowComments(!showComments)} className="viewComments">
                    {showComments ? "Hide" : "View"} all {commentsToShow.length} comment{comments.length !== 1 && 's'} </h4>
                {
                    showComments &&
                    (
                        <div id={"listOfComments"}>
                            {
                                commentsToShow.map(singleComment => (
                                    <div style={{
                                        marginBottom: '4px',
                                        display: 'flex',
                                        justifyContent: 'left',
                            ...
                            <hr style={{width: '60%', margin: '10px 0 5px 0', color: 'lightgray'}}/>
                        </div>
                    )
                }
                <label>
                    New Comment<br/>
                    <span>
                        <input style={{width: '40%', height: '25px'}} value={commentText}
                               onChange={(event) => setCommentText(event.target.value)}/>
                        <button style={{
                            width: '50px',
                            height: '25px',
                            marginLeft: '5px',
                            border: 'solid #bcc7ff 2px',
                            borderRadius: '5px',
